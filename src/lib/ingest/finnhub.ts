/**
 * Optional second market-data source (spec §2, §7). Exists only to produce
 ...
 * second source is exactly the "app works with one source" path (spec §7).
 */
export async function getFinnhubQuote(symbol: string): Promise<FinnhubQuote | null> {
  ...
  try {
    ...
    // failure of the app; it just means one source for this tick (spec §7).
    return null;
  }
}
